...
def serpapi_search(config_path, query, section='Serp_API', option='api_key', , location='Hong Kong', hl='zh-cn',
                   gl='cn',
                   tbs=None, tbm=None,
                   num=30, ):
    """
    ...
    args:
        ...
        hl:Parameter defines the country to use for the Google search. It's a two-letter country code. (e.g., us for the United States, uk for United Kingdom, or fr for France)
        gl:Parameter defines the language to use for the Google search. It's a two-letter language code. (e.g., en for English, es for Spanish, or fr for French). Head to the Google languages page for a full list of supported Google languages.
        num:Parameter defines the maximum number of results to return. (e.g., 10 (default) returns 10 results
        tbs:(to be searched) parameter defines advanced search parameters that aren't possible in the regular query field. (e.g., advanced search for patents, dates, news, videos, images, apps, or text contents).
...
if __name__ == "__main__":
    ...
    # api_key = get_api_key(config_path, 'Serp_API', 'api_key')
    # print(api_key)
    search_result = serpapi_search(config_path, 'Serp_API', 'api_key', query, 'Hong Kong', 'zh-cn', 'cn', )
    print(search_result)