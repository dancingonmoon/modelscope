...
def GLM_callFunc_Async(zhipuai_client, question, query, LLM_model="glm-4-air", web_search_enable=True, web_search_result_show=False,
                       time_threshold: Union[float,None]=5):
    """
    ...
    time_threshold: time_threshold内,持续检索模型异步输出,直到获得模型输出;达到time_threshold,仍没有获得模型输出,则给微信服务器输出success响应;缺省值为5秒;当time_threshold为None时,即不再做时间限制,不做判断
    return:
        out_text: 模型输出
    ...
    if time_threshold is not None: # 允许时间阈值
        while True:
...
def GLM_callFunc_SSE_SYN(zhipuai_client, question, query, LLM_model="glm-4-air", stream=True, web_search_enable=True, web_search_result_show=False,
                         time_threshold =5):
    """
    实现GLM流式调用,开启web_search工具;在time_threshold时间(微信服务器限定响应时间为5秒), 输出模型SSE所能获得的全部输出;同步非流式,不再加时间限制
    zhipuai_client: 初始化之后的ZhipuAI客户端
    LLM_model: glm-4-0520, glm-4 , glm-4-air, glm-4-airx,  glm-4-flash, 或glm-3-
    stream: 缺省为Ture,即SSE流式;当False时, 会响应结束后一次性输出;此时time_threshold不再适用了;
    web_search_enable: 缺省打开
    web_search_result_show: 是否输出web_search结果,缺省关闭; web_search结果为列表,包含每个网页链接字典,其keys: {content, icon,link,media,refer};
    ...
    if stream:
        ...
        try:
            ...
            if web_search_result_show:
                out_search = response.web_search
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    out = GLM_callFunc_SSE_SYN(zhipuai_client, question, query, stream=False, web_search_enable=web_search_enable,
                               web_search_result_show=web_search_result_show, time_threshold=time_threshold)
