...
class EchoTextHandler(dingtalk_stream.ChatbotHandler):
    ...
    async def process(self, callback: dingtalk_stream.CallbackMessage):
        ...
        query = question
        text = '问题收到,请等待我调用大模型来回答:'
        self.reply_text(text, incoming_message)
        text = chatGLM_RAG_generate(question=question, query=query,search_engine='Baidu')
        self.reply_text(text, incoming_message)
...
def main():
    ...
    config_path = r"E:/Python_WorkSpace/config/DingTalk_APP.ini"
    client_id, client_secret = config_read(config_path)
